...
DBGrid.Columns.Add;
DBGrid.Columns[0].Title.Caption:='Дата замены';
DBGrid.Columns[0].FieldName:='date';
DBGrid.Columns[0].Width:=100;
DBGrid.Columns.Add;
DBGrid.Columns[1].Title.Caption:='Старый номер';
DBGrid.Columns[1].FieldName:='old_n';
DBGrid.Columns[1].Width:=100;
DBGrid.Columns.Add;
DBGrid.Columns[2].Title.Caption:='Новый номер';
...
Columns[0].Width:=70;
Columns.Add;
Columns[1].Title.Caption:='водитель';
Columns[1].FieldName:='driver';
Columns[1].Width:=150;
Columns.Add;
Columns[2].Title.Caption:='виновен';
Columns[2].FieldName:='guilty_str';
Columns[2].Width:=70;
Columns.Add;
Columns[3].Title.Caption:='кол-во пострадавших';
Columns[3].FieldName:='injured';
Columns[3].Width:=120;
Columns.Add;
Columns[4].Title.Caption:='сумма ущерба';
Columns[4].FieldName:='amount_damages';
Columns[4].Width:=100;
Columns.Add;
Columns[5].Title.Caption:='ремонт в СТО';
...
begin
      Main.ATXShowMessage('Не правильно заполнено Гос №!');
      Abort;
end else
begin
...
id_akb:integer;
begin
  if Main.ATXMessageDlg('Вы уверены, что хотите удалить данный аккамулятор?',
    2,mtConfirmation) = mrYes then
begin
...
{end else
begin
  Main.ATXShowMessage('Ремонт в СТО не может быть пустым!');
end;}
...
if Main.ATXMessageDlg('Экспортировать данный автотранспорт в БД страхование?',
    2,mtConfirmation) = mrYes then
    begin
...
if res=-1 then
begin
if Main.ATXMessageDlg('Автотранспорт '+Mark_box.Text+' '+Model_edit.Text+' гос.номер '+Numb1_edit.EditText+' г.в. '+Year_edit.EditText+' поставленный на учет '+InAc_edit.EditText+' уже существует в БД. Добавить новый автотранспорт?',
    2,mtConfirmation) = mrYes then
    begin
    dm.StoredProc1.Close;
dm.StoredProc1.Parameters.Clear;
...
id_shin:integer;
begin
  if Main.ATXMessageDlg('Вы уверены, что хотите удалить данную шину? (Удаление приведет к удалению всей истории шины)',
    2,mtConfirmation) = mrYes then
begin
id_shin:=dm.StoredProc_shin.Fields[0].AsInteger;
if dm.StoredProc_shin.Active then
...
ListShinProbeg(dm.StoredProc_shin.Fields[0].AsInteger,dm.tAvto.Fields[0].AsInteger,'все');
...
StringGrid1.Cells[0,0]:='общий пробег';
...
begin
if ComboBox2.Text='' then ComboBox2.Text:='все';
...
Parameters[3].DataType:=ftInteger;
if ComboBox_kv.Text='I квартал' then
Parameters[3].Value:=1;
if ComboBox_kv.Text='II квартал' then
Parameters[3].Value:=2;
if ComboBox_kv.Text='III квартал' then
Parameters[3].Value:=3;
if ComboBox_kv.Text='IV квартал' then
...
ComboBox_kv.Text:='';
RxCalcEdit1.Value:=0;
LbButton19.Caption:='Добавить';
LbButton19.Enabled:=false;
end else
Main.ATXMessageDlg('Не заполнено поле "квартал"!',1,mtError);
end;
...
//ListShinProbeg(a,0,'все');
Main.CreateReportShine;
//ListShinProbeg(a,b,'все');
end;
