...
State_p,State_priz,UotAc_p,life_p,id_dislok,arenda,spisat,spisat_Next,fuel_p,notATX:integer;
str:String;
Ch:TCheckBox;
n_i:byte;
error:boolean;
...
if Spisat_Next_box.Checked then spisat_Next:=1 else spisat_Next:=-1;
...
Spisat_Next_box.Visible:=true;
Caption:='Выборка для отчета "Инвентаризация"';
end;
procedure Tfrmreport.InventarSimple;
begin
what:=63;
d1L.Visible:=false;
d2L.Visible:=false;
...
TnotATX.Visible:=true;
Spisat_Next_box.Visible:=true;
Caption:='Выборка для отчета "Инвентаризация" (без группировки)';
end;
