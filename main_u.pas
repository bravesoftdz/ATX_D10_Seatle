...
begin
  ...
  try
    ...
    ATXMessageDlg('Нет соединения с сервером ! Обратитесь к администратору !',1, mtError);
    Application.Terminate;
  end;
end;  }
end;
...
12:begin frxReport1.ReportOptions.Name:='Марка ГСМ';
end;
13:begin frxReport1.ReportOptions.Name:='Вид ремонта';
end;
14:begin frxReport1.ReportOptions.Name:='Место ремонта';
end;
...
frxReport1.ReportOptions.Name:='Шины';
whatReport:=7;
//frxReport1.OnObjectClick := nil;
frxReport1.OnMouseOverObject := nil;
frxReport1.Preview := nil;
...
frxReport1.ReportOptions.Name:='АКБ';
whatReport:=8;
//frxReport1.OnObjectClick := nil;
frxReport1.OnMouseOverObject := nil;
frxReport1.Preview := nil;
...
frxReport1.ReportOptions.Name:='Акт закрепления';
whatReport:=91;
frxReport1.OnMouseOverObject := nil;
frxReport1.Preview := nil;
if frxReport1.PrepareReport then
...
case what of
0:frxReport1.ReportOptions.Name:='Акт инвентаризации';
1:frxReport1.ReportOptions.Name:='Эксплуатационные показатели';
2:frxReport1.ReportOptions.Name:='Эксплуатационная карточка 1';
3:frxReport1.ReportOptions.Name:='Эксплуатационная карточка 2';
11:frxReport1.ReportOptions.Name:='Эксплуатационные показатели';
12:frxReport1.ReportOptions.Name:='Годовой план эксплуатации автотранспорта';
13:frxReport1.ReportOptions.Name:='Отчет по шинам';
5:frxReport1.ReportOptions.Name:='Расчет линейных норм ГСМ';
51:frxReport1.ReportOptions.Name:='Расчет линейных норм ГСМ по марке';
10:frxReport1.ReportOptions.Name:='Отчет по ремонту';
41:frxReport1.ReportOptions.Name:='Финансирование ГСМ';
61:frxReport1.ReportOptions.Name:='Необходимо пройти ТО в текущем году';
62:frxReport1.ReportOptions.Name:='Отсутствие ТО более 3 месяцев';
71:frxReport1.ReportOptions.Name:='Отчет по маслу';
...
begin
//формируем штатку
if ATXMessageDlg('При формировании штатного расписания старое расписание будет уничтожено!. Продолжить формирование?',
...
//вместо процедуры, т.к. она долгая
q2.Close;
q2.SQL.Clear;
...
//конец вместо процедуры, т.к. она долгая
frmLoadData.Close;
ATXShowMessage('Штатное расписание сформировано!');
 end;
end;
end;
end;
...
frxReport1.ReportOptions.Name:='Штатное расписание';
//frxReport1.OnObjectClick := nil;
frxReport1.OnMouseOverObject := nil;
frxReport1.Preview := nil;
if frxReport1.PrepareReport then
...
frReport1.Title:='Штатное расписание';
...
begin
     ...
     begin
          ...
          try
            Reg.RootKey := HKEY_CLASSES_ROOT;
              if Reg.OpenKey('\CLSID\{96A7934F-0C7A-4C35-930C-012A5A8EF7DB}', True) then
               ...
               ATXShowMessage('Активация прошла успешно!');
          except
          end;
            Reg.CloseKey;
            Reg.Free;
      ...
      begin
          ATXShowMessage('Неверный код активации!');
      end;
      SActivate.Free;
     end
end;
...
11:begin
  ...
  begin
    if frmreport2.gsm_box.Checked then
       Value := 'Тип топлива: '+Trim(frmreport2.db_gsm.Text)
    else
       Value := 'Тип топлива: все';
  end;
end;
71,72:begin
  if VarName = 'date2' then
...
if VarName='year' then
 Value:=frmreport.ComboBox1.Text+' год';
end;
6:begin
...
if VarName='power' then Value:=Tfrmavtocard(ActiveMDIChild).DBEdit_power_l.Text+' л.с./'+Tfrmavtocard(ActiveMDIChild).DBEdit_power_k.Text+' кВт';
...
if VarName='insur1' then Value:=Tfrmavtocard(ActiveMDIChild).Insur_edit.Text+' от '+Tfrmavtocard(ActiveMDIChild).date_insur.Text;
...
if VarName='year' then Value:=Tfrmavtocard(ActiveMDIChild).Year_edit.Text;
if VarName='pts' then Value:=Tfrmavtocard(ActiveMDIChild).NPts_edit.Text+' от '+Tfrmavtocard(ActiveMDIChild).date_texpasport.Text;
...
if VarName='osago' then Value:=Tfrmavtocard(ActiveMDIChild).Insur_edit.Text+' от '+Tfrmavtocard(ActiveMDIChild).date_insur.Text;
...
if (Trim(frmreport.d1R.Text)<>'.  .') and (Trim(frmreport.d1R.Text)<>'.  .') then
 Value:='Отчет по ремонту автотранспорта с '+frmreport.d1R.Text+' по '+frmreport.d2R.Text
else
Value:='Отчет по ремонту автотранспорта на '+DateToStr(Now());
first_fr:=true;
for n_i:=1 to 26 do
begin
