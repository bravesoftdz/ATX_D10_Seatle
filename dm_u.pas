...
type
  Tdm = class(TDataModule)
    ...
    dss_hist: TDataSource;
    tHistoryExchange: TADOTable;
    dss_pered: TDataSource;
    tPeredacha: TADOTable;
    tSettingProgramm: TADOTable;
    tBaseNormGsm: TADOTable;
    dss_basenorm: TDataSource;
    tMarkGSM: TADOTable;
    dss_markGsm: TDataSource;
    tYearExplBig: TADOTable;
    tYearExpl: TADOTable;
    dss_yearexpl: TDataSource;
    dss_yearexplbig: TDataSource;
    tPlaceRemont: TADOTable;
    tVidRemont: TADOTable;
    dss_placerem: TDataSource;
    dss_vidrem: TDataSource;
    StoredProc_akb: TADOStoredProc;
    dss_akb: TDataSource;
    dss_shin: TDataSource;
    StoredProc_shin: TADOStoredProc;
    storedProc_ShinProbeg: TADOStoredProc;
    dss_ShinProbeg: TDataSource;
    ...
    tPreServ: TADOTable;
    tShina_R: TADOTable;
    tShina_Konst: TADOTable;
    tShina_Type: TADOTable;
    dss_sh_r: TDataSource;
    dss_sh_k: TDataSource;
    dss_sh_t: TDataSource;
    tShina_Limit: TADOTable;
    dss_shina_limit: TDataSource;
    storedProc_ShinProbegShort: TADOStoredProc;
    dss_ShinProbegShort: TDataSource;
    ADOQuery3: TADOQuery;
  ...
  procedure SessionControl(what:integer;ip,username:String);
  procedure BeginTR;
  procedure CommitTR;
  procedure RollBTR;
...
procedure Tdm.SessionControl(what:integer;ip,username:String);
begin
try
if ADO.Connected then begin
StoredProc1.Parameters.Clear;
StoredProc1.ProcedureName:='SessionControl;1';
StoredProc1.Parameters.AddParameter;
StoredProc1.Parameters[0].Name:='@what';
StoredProc1.Parameters[0].DataType:=ftInteger;
StoredProc1.Parameters.ParamByName('@what').Value:=what;
StoredProc1.Parameters.AddParameter;
StoredProc1.Parameters[1].Name:='@ip';
StoredProc1.Parameters[1].DataType:=ftString;
StoredProc1.Parameters.ParamByName('@ip').Value:=ip;
StoredProc1.Parameters.AddParameter;
StoredProc1.Parameters[2].Name:='@username';
StoredProc1.Parameters[2].DataType:=ftString;
StoredProc1.Parameters.ParamByName('@username').Value:=username;
...
try
    ...
    tHistoryExchange.Open;tHistoryExchange.Sort:='date';
    tPeredacha.Open;tPeredacha.Sort:='date';
    tGroup_used.Open;tGroup_used.Sort:='name';
    dm.tMarkGSM.Open;dm.tMarkGSM.Sort:='name';
    tInsureCompany.Open;tInsureCompany.Sort:='name';
    tShina_R.Open;tShina_R.Sort:='name';
    tShina_Konst.Open;tShina_Konst.Sort:='name';
    tShina_Type.Open;tShina_Type.Sort:='name';
...
if DataSet.RecNo=1 then
begin
frmLidBaseGsm.LbB_First.Enabled:=false;
frmLidBaseGsm.LbB_Prior.Enabled:=false;
end
else
begin
frmLidBaseGsm.LbB_First.Enabled:=true;
frmLidBaseGsm.LbB_Prior.Enabled:=true;
end;
if DataSet.RecNo=DataSet.RecordCount then
begin
frmLidBaseGsm.LbB_Next.Enabled:=false;
frmLidBaseGsm.LbB_Last.Enabled:=false;
end
else
begin
frmLidBaseGsm.LbB_Next.Enabled:=true;
frmLidBaseGsm.LbB_Last.Enabled:=true;
end;
end;
...
if DataSet.RecNo=1 then
begin
frmFinGSM.LbB_First.Enabled:=false;
frmFinGSM.LbB_Prior.Enabled:=false;
end
else
begin
frmFinGSM.LbB_First.Enabled:=true;
frmFinGSM.LbB_Prior.Enabled:=true;
end;
if DataSet.RecNo=DataSet.RecordCount then
begin
frmFinGSM.LbB_Next.Enabled:=false;
frmFinGSM.LbB_Last.Enabled:=false;
end
else
begin
frmFinGSM.LbB_Next.Enabled:=true;
frmFinGSM.LbB_Last.Enabled:=true;
end;
end;
